...
A multimodal machine learning pipeline that combines NLP and CV. The pipeline accepts a text message and an image, identifies which animal is referenced in the text, and verifies whether the image contains that animal — returning a single boolean value.
...
│   └── ner_dataset.json        # Generated NER training data (not included in repo)
├── models/                     # Saved model weights (.pth and HF format) (not included in repo)
├── notebooks/
│   ├── data_loading.ipynb      # Dataset download and preparation
...
Generate the synthetic NER training dataset from the `task_2/` root:
...
All training commands should be run from the `task_2/` root.
...
All inference commands should be run from the `task_2/` root.
...
Both scripts support `--device -1` (CPU) or `--device 0` (GPU/MPS) and `--log_level DEBUG/INFO/WARNING/ERROR`.

---

## Pipeline

> **Note:** Always run pipeline commands from the `task_2/` root with `PYTHONPATH=.` so that `src` is resolvable as a package.
...
| `--device` | auto | `cpu`, `cuda`, or `mps`. Auto-detected if omitted. |
...
=============================================
  Text  : There is a beautiful cow standing in the field.
  Image : data/raw-img/cow/<filename>.jpeg
=============================================
  RESULT: True
=============================================
```
...
| File | Description |
|------|-------------|
| `notebooks/data_loading.ipynb` | Dataset download, extraction, and Italian-to-English folder renaming. |
...
| `src/generate_data.py` | Synthetic NER dataset generator (1600 sentences, 8 styles, balanced per class). |
...
### `data_loading.ipynb`
Handles the full dataset setup: Kaggle download, extraction to `task_2/data/`, and automatic renaming of Italian folder names to English class names.
